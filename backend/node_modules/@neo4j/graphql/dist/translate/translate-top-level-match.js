...
function createMatchClause({ matchNode, matchPattern, node, context, operation, where, }) {
    ...
    let matchClause = new cypher_builder_1.default.Match(matchPattern);
    let whereOperators = [];
...
function createFulltextMatchClause(matchNode, whereInput, node, context) {
    ...
    if (whereInput?.[constants_1.SCORE_FIELD]) {
        if (whereInput[constants_1.SCORE_FIELD].min || whereInput[constants_1.SCORE_FIELD].min === 0) {
            const scoreMinOp = cypher_builder_1.default.gte(scoreVar, new cypher_builder_1.default.Param(whereInput[constants_1.SCORE_FIELD].min));
            if (scoreMinOp)
                whereOperators.push(scoreMinOp);
        }
        if (whereInput[constants_1.SCORE_FIELD].max || whereInput[constants_1.SCORE_FIELD].max === 0) {
            const scoreMaxOp = cypher_builder_1.default.lte(scoreVar, new cypher_builder_1.default.Param(whereInput[constants_1.SCORE_FIELD].max));
            if (scoreMaxOp)
