...
function createConnectOrCreateAndParams({ input, varName, parentVar, relationField, refNode, node, context, withVars, callbackBucket, }) {
    (0, utils_1.asArray)(input).forEach((connectOrCreateItem) => {
        (0, assert_non_ambiguous_update_1.assertNonAmbiguousUpdate)(refNode, connectOrCreateItem.onCreate?.node ?? {});
    });
    // todo: add create
    ...
    return (0, build_clause_1.buildClause)(query, { context, prefix: `${varName}_` });
}
...
function mergeStatement({ input, refNode, context, relationField, parentNode, varName, callbackBucket, }) {
    ...
    const relationshipMerge = new cypher_builder_1.default.Merge(relationshipPattern).onCreateSet(...onCreateRelationshipParams);
    let withClause;
