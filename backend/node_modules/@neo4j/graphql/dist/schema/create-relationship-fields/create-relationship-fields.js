...
composeNode, subgraph, userDefinedFieldDirectives, seenRelationshipPropertiesTypes, userDefinedDirectivesForNode, userDefinedFieldDirectivesForNode, features, }) {
    ...
    relationships.forEach((relationshipAdapter) => {
        ...
        if (composeNode instanceof graphql_compose_1.ObjectTypeComposer) {
            ...
            const aggregationTypeObject = fieldAggregationComposer.createAggregationTypeObject(relationshipAdapter, features);
            const aggregationFieldsBaseArgs = {
                where: relationshipTarget.operations.whereInputTypeName,
            };
            const aggregationFieldsArgs = (0, directed_argument_1.addDirectedArgument)(aggregationFieldsBaseArgs, relationshipAdapter, features);
            if (relationshipAdapter.aggregate) {
                if ((0, utils_1.shouldAddDeprecatedFields)(features, "deprecatedAggregateOperations")) {
                    composeNode.addFields({
                        [relationshipAdapter.operations.aggregateTypeName]: {
                            type: aggregationTypeObject,
                            args: aggregationFieldsArgs,
                            directives: deprecatedDirectives.length > 0
                                ? deprecatedDirectives
                                : [(0, constants_2.DEPRECATE_NESTED_AGGREGATION)(relationshipAdapter)],
                        },
                    });
                }
            }
...
features, }) {
    ...
    if (relationshipAdapter.target instanceof InterfaceEntityAdapter_1.InterfaceEntityAdapter) {
        (0, create_input_1.withFieldInputType)({ relationshipAdapter, composer, userDefinedFieldDirectives, features });
    }
    else {
