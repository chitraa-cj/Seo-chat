    ...
    for (const attribute of aggregableAttributes) {
        ...
        if (objectTypeComposer) {
            ...
            if (attribute.typeHelper.isID()) {
                if ((0, utils_1.shouldAddDeprecatedFields)(features, "idAggregations")) {
                    aggregableFields[attribute.name] = {
                        type: objectTypeComposer.NonNull,
                        directives: [constants_2.DEPRECATE_ID_AGGREGATION],
                    };
                }
                continue;
            }
...
composer, userDefinedDirectivesOnTargetFields, features, }) {
    ...
    if ((0, utils_1.shouldAddDeprecatedFields)(features, "implicitEqualFilters")) {
        aggregateWhereInput.addFields({
            count: { type: graphql_1.GraphQLInt, directives: [constants_2.DEPRECATE_IMPLICIT_EQUAL_FILTERS] },
        });
    }
    aggregateWhereInput.addFields({
...
function makeAggregationFields(attributes, userDefinedDirectivesOnTargetFields, features) {
    ...
    for (const attribute of attributes) {
        addAggregationFieldsByType(attribute, userDefinedDirectivesOnTargetFields?.get(attribute.name), fields, features);
    }
    return fields;
}
// TODO: refactor this by introducing specialized Adapters
function addAggregationFieldsByType(attribute, directivesOnField, fields, features) {
    const deprecatedDirectives = (0, to_compose_1.graphqlDirectivesToCompose)((directivesOnField || []).filter((d) => d.name.value === constants_1.DEPRECATED));
    ...
    for (const operator of constants_1.AGGREGATION_COMPARISON_OPERATORS) {
        ...
        if (attribute.typeHelper.isID()) {
            if ((0, utils_1.shouldAddDeprecatedFields)(features, "idAggregations")) {
                fields[`${attribute.name}_MIN_${operator}`] = {
                    type: attribute.getTypeName(),
                    directives: deprecatedDirectives.length ? deprecatedDirectives : [constants_2.DEPRECATE_ID_AGGREGATION],
                };
                fields[`${attribute.name}_MAX_${operator}`] = {
                    type: attribute.getTypeName(),
                    directives: deprecatedDirectives.length ? deprecatedDirectives : [constants_2.DEPRECATE_ID_AGGREGATION],
